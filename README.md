...
> 万が一環境変数が未設定の場合でも、安全性を最優先にするために、プログラム上のデフォルト動作モードは **`single`（個人・家庭用）モード**（一般登録は無効）になります。
> 公開用の SaaS インスタンス（`multi` モード）としてデプロイしたい場合は、明示的に `APP_MODE=multi` および `ENABLE_SIGNUP=true` を環境変数に定義してください。

| 環境変数 | 設定値の例 | デフォルト値（未指定時） | 説明 |
| :--- | :--- | :--- | :--- |
| `APP_MODE` / `NEXT_PUBLIC_APP_MODE` | `single` \| `multi` | `single` | アプリケーションの動作モード。`single` は家庭用、`multi` は公開用。 |
| `ENABLE_SIGNUP` / `NEXT_PUBLIC_ENABLE_SIGNUP` | `true` \| `false` | `false` (APP_MODEがsingleの場合)<br>`true` (APP_MODEがmultiの場合) | 一般のサインアップ（新規アカウント作成、匿名ログイン含む）を有効にするか。`single` では `false` 推奨。 |
